...
/**
 * 2. Hugging Face Inference API / Router (Llama-3.3-70B / Qwen-2.5-72B)
 */
async function callHuggingFace(systemPrompt: string, userMessage: string): Promise<string | null> {
  ...
  for (const model of models) {
    try {
      // Use standard OpenAI-compatible Hugging Face router endpoint
      ...
      if (res.ok) {
        ...
        const errText = await res.text();
        console.warn(`⚠️ [AI Engine: Hugging Face ${model} HTTP ${res.status}]:`, errText.slice(0, 100));
...
/**
 * 3. Groq Cloud Ultra-Fast Provider (Llama-3.3-70B / Llama-3.1-8B)
 */
