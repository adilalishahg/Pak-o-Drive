...
export interface BlogPostTemplateProps {
  ...
  tocHeadings: Array<{ title: string; id: string }>;
...
export function BlogPostTemplate({
  ...
  const isAuto = hub === 'auto';
  const hubTitle = isAuto ? 'Auto Journal' : 'Journal';
  const hubPath = isAuto ? '/auto' : '/blog';
  const defaultCategory = isAuto ? 'Auto Guide' : 'Featured Article';
  const authorTitle = post.author || (isAuto ? 'Pak-o-Drive Specialist' : 'Pak-o-Drive Specialist');
  const authorRole = isAuto ? 'Automotive Specialist' : 'Editorial Specialist';
  const authorBadge = isAuto ? 'Verified Automotive Specialist' : 'Verified Editorial Lead';

  const defaultTags = isAuto
    ? ['Summer Heat', 'Engine Oils', 'Mehran Mileage', 'Motorway Fog', 'Car Care', 'Tuning', 'AC Cooling']
    : ['Technology', 'AI', 'Car Care', 'Maintenance', 'Fuel', 'Gadgets', 'Safety', 'Summer Hacks'];

  const newsletterDesc = isAuto
    ? 'Signup and receive car maintenance tips and exclusive COD deals every week.'
    : 'Signup and receive recent articles and deals in your inbox every week.';
      ...
      <header className="relative bg-gradient-to-b from-slate-50/90 via-rose-50/20 to-white border-b border-slate-200/80 pt-8 sm:pt-12 pb-10 sm:pb-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          <nav aria-label="Breadcrumb" className="inline-flex items-center gap-1.5 text-xs text-slate-500 font-medium mb-4 px-3.5 py-1.5 rounded-full bg-white/90 backdrop-blur-xs border border-slate-200/80 shadow-xs">
            ...
            <Link href={hubPath} className="hover:text-rose-500 transition-colors text-decoration-none">
              {hubTitle}
            </Link>
            <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
            <span className="text-slate-800 font-semibold truncate max-w-[200px] sm:max-w-xs">{post.category || (isAuto ? 'Automotive' : 'Tech & AI')}</span>
          </nav>
            ...
            <div className="flex items-center gap-3 text-left">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-slate-400 text-[11px] mt-0.5">
                  ...
                  <span>•</span>
                  <span>{isAuto ? 'Pakistani Automotive Desk' : 'Pak-o-Drive Editorial Desk'}</span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-14 items-start">
            ...
            {post.tags && post.tags.length > 0 && (
              <div className="pt-6 border-t border-slate-100 flex flex-wrap items-center gap-2">
                ...
                {post.tags.map((tag: string) => (
                  <Link
                    key={tag}
                    href={`${hubPath}?tag=${encodeURIComponent(tag)}`}
                  ...
                  <div className="text-center sm:text-left flex-1 pt-1">
                    ...
                    <p className="text-xs sm:text-sm text-slate-600 mt-2 leading-relaxed">
                      {isAuto
                        ...
                        : 'Pak-o-Drive senior technical researcher and car enthusiast. Covering practical automotive tech, accessories, and maintenance hacks tested for local driving conditions.'}
                    </p>
                  </div>
                </div>
          ...
          <aside className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            {/* Sidebar Author Profile Card (Desktop Only) */}
              ...
              <div className="px-6 pb-6 text-center relative -mt-8">
                ...
                <p className="text-xs text-slate-600 leading-relaxed mt-2.5">
                  {isAuto
                    ...
                    : 'Practical guides & tested automotive product reviews for Pakistani car owners.'}
                </p>
              ...
              <div className="p-5 sm:p-6 rounded-2xl border border-slate-200/90 bg-white shadow-xs">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                  <h5 className="font-serif font-extrabold text-sm sm:text-base text-slate-900 m-0">
                    {isAuto ? 'Recent Guides' : 'Related Articles'}
                  </h5>
            ...
            <BlogNewsletterBox description={newsletterDesc} />

            {/* Tag Cloud Widget (Desktop Only) */}
